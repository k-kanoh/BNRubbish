...
namespace BerorinApp
{
    public class MenuStripTemplate : IList<MenuStripTemplate>
    {
        ...
        public Func<bool> CheckOnAction { get; set; }
        ...
        public MenuStripTemplate(string name, Action method, Func<bool> canExecute, Func<bool> checkOnAction, Keys? shortcut = null)
        {
            Name = name;
            Method = method;
            CanExecute = canExecute;
            CheckOnAction = checkOnAction;
                ...
                else if (item.Any())
                {
                    ...
                    menu.DropDownOpened += (s, e) =>
                    {
                        for (int i = 0; i < menu.DropDownItems.Count; i++)
                        {
                            ...
                            if (menu.DropDownItems[i] is ToolStripMenuItem menuItem && item[i].CheckOnAction != null)
                                menuItem.Checked = item[i].CheckOnAction.Invoke();
                        }
                    };
